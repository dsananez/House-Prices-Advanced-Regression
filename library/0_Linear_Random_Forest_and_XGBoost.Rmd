---
title: "Linear Model, Random Forest and XGBoost0"
output: html_document
---
##Analyzing Real Estate In Numbers## <!-- Based on Stephanie Kirmer's Approach on Kaggle -->

Buying a house could be one of the most difficult and stressing moments in life. There are so many things to consider (bedrooms, neighborhood, kitchen, lot area, garage size, etc). However, using statistical models we can determine what really influences the value of a house.
...
*2. Cleaning and reformatting some variables
*3. Modeling (Linear, Random Forest and XGBoost)
*4. Predictions


###Clean the Data

So, lets take a look at the variables in the dataset (description available [here](https://www.kaggle.com/c/house-prices-advanced-regression-techniques/download/data_description.txt)):
...
The dataset comes with many character variables (first 3 are shown). These variables aren't very useful for modeling.

```{r formatting}
print(c("MSZoning", table(train$MSZoning)))
print(c("Street", table(train$Street)))
print(c("LotShape", table(train$LotShape)))
```

First, we have to convert them to factors or numeric variables. But, are they ordinal or cardinal?
